...
// ─── Synchronous L1-only helpers (kept for internal/observability use) ────────

/** L1-only read. Prefer getCached() in request paths so cold starts hit L2. */
export function cacheGet<T>(key: string): T | undefined {
  return l1Get<T>(key)
}

/** L1-only write. Prefer setCached() in request paths so the durable tier is populated. */
